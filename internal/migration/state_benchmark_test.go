		...
		for _, scheme := range []string{rawdb.HashScheme, rawdb.PathScheme} {
			...
			b.Run(compression+"/"+scheme+"/generate-trie-reference", func(b *testing.B) {
				...
				for range b.N {
					...
					if _, err := verifyTargetDatabase(context.Background(), output, scheme, targetConfig{engine: DBEnginePebbleV2, layout: LayoutGeth}, exported.Manifest.Source, result.State, 128, 128, reporter, trieNodeIndexOptions{Parent: filepath.Dir(output), CacheMB: 128, Handles: 128}); err != nil {
						b.Fatal(err)
					}
				}
			})
