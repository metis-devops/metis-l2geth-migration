...
It migrates only the current accounts, storage, and contract code committed to by the `LastBlock` header. It does not migrate historical state, block bodies, transactions, receipts, chain configuration, or trie preimages. The resulting artifact is not a complete `chaindata` directory from which an unmodified geth node can boot.
...
7. Compare the database inventory with the exact number of trie nodes and unique code entries reached from the target root, and reject orphan nodes, unreferenced code, malformed path metadata, block, head, receipt, history, or other non-state data.
